...
//	time.innerHTML =  " No More Buses Today 1" ;
	out =  " No More Buses Today 1" ;
    }
    else if ( target_time - pre_time > 180 ) {
//	time.innerHTML = " There are no buses in the near 3 hours.";
	out = " There are no buses in the near 3 hours.";
    }
    else {
	out = "Bus      => " + target_h + ":" + target_m + "<br/>Present time => " + pre_time_h + ":" + pre_time_m + "<br/> 残り      => " + diff + "min";
    }
    return out;
}
...
//データベースを増やしていけばok
	...
	if ( stop == "慶応大学/(急行)湘南台駅西口" ) {
		...
		bus_stop =  get_bus_stop(keiouniv_syounandai_kyuukou_sat) + " // " + stop;
            }
            else {
		bus_stop =  get_bus_stop(keiouniv_syounandai_kyuukou_weekday) + " // " + stop;
            }
	}
	else if ( stop == "慶応大学/湘南台駅西口" ) {
            if ( Day == 0 ) { ///SUN
		bus_stop =  get_bus_stop(keiouniv_syounandai_norm_sun) + " // " + stop;
            }
            else if ( Day == 6 ) { //SAT
		bus_stop =  get_bus_stop(keiouniv_syounandai_norm_sat) + " // " + stop;
            }
            else {
		...
		bus_stop =  get_bus_stop(keiomain_syounandai_norm_sat) + " // " + stop;
            }
            else {
